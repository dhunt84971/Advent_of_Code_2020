...
for (var i=0; i<datas.length; i++){
	let passFiltered = datas[i][2];
	let filterChar = datas[i][1];
	let minCount = parseInt(datas[i][0].split("-")[0]);
	let maxCount = parseInt(datas[i][0].split("-")[1]);
	let charCount = 0;
	for (let letter = 0; letter < passFiltered.length; letter ++){
		if (passFiltered.charAt(letter) == filterChar) charCount++;
	}
	if (charCount >= minCount && charCount <= maxCount) answer++;
}
